...
public class Program {
	...
	public static void main(String[] args) {
		...
		if (result >= 7 && result <= 10 && absences <= 36) {
			System.out.printf("Aprovado. Média: %.2f | Faltas: %d", result, absences);
		} else if (result >= 4 && result <= 7 && absences <= 36) {
			System.out.printf("Exame final. Média: %.2f | Faltas: %d", result, absences);
		} else if (result < 4 && absences > 36) {
			System.out.printf("Reprovado. Média: %.2f | Faltas: %d", result, absences);
		} else {
